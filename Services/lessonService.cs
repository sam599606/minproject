...
namespace minproject.Services.lessonService
{
    public class lessonService
    {
        ...
        {
            string sql = @"UPDATE Lesson SET Account=@Account,Type=@Type,Price=@Price,Content=@Content,Video=@Video,Year=@Year,EditTime=@EditTime WHERE LessonID = @Id";
            try
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@Account", Editlesson.Account);
